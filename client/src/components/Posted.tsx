...
useEffect(() => {
  axios.get("http://localhost:5500/getdb").then((res) => setList(res.data.message)).catch((e) => console.error(e))
}, [])
...
<button className='w-full my-4 text-red-500 hover:text-white hover:bg-red-500 rounded-lg font-bold' onClick={() => DelReq(user._id)}>delete post</button>
</div>
</>   }
     </div>
}
